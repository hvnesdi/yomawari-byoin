# Two-pass runner: replace architecture (walls/floors/ceilings/columns) + capture.
...
Run-Pass -method 'HospitalPackArchitecture.RunBatch' -logName 'arch_apply.log'
if ($script:LastExit -ne 0) { exit $script:LastExit }
Run-Pass -method 'HospitalPackArchitecture.CaptureFive' -logName 'arch_capture.log'
