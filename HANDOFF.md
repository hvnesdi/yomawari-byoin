...
| 蛍光灯の位置 | 天井がいちばん明るい（逆さま） | 器具が y=2.88 決め打ちで天井（下面2.80/2.60）に埋まり、光源も天井より上にあった |
...
全フロアでプレイテスト 24/24 PASS。地下は蛍光灯1本が天井を照らすだけで、
...
6. **人物の造形が最大の弱点**。動きは付いたが（項目0）、顔も皮膚も衣服のしわも無い。
   背景が実写素材と焼き込んだ間接光で上がったぶん、**落差がいちばん目につく**。
   → Mixamo で解決する
...
**高さを決め打ちしない。** 蛍光灯の器具が y=2.88 に固定で置かれていたが、
天井の下面は 1F/2F が 2.80、3F は 2.60。全部が天井に埋まっていた。
光源も天井より上にあり、**部屋ではなく天井板を照らしていた**
（画面でいちばん明るいのが天井、という逆さまの絵になっていた）。
`M18LightFixturePass` は光源ごとに真上の天井を測ってから吊る。
フロアでも部屋でも天井高が違うので、1つの数値で済ませられない。
...
| 照明の位置を測る | `LightDiagnostics` | 約1分 |
| 蛍光灯を吊り直す（要ベイク） | `M18LightFixturePass` → `M9BakedLightingPass.BakeAll` | 約7分 |
