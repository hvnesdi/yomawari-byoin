...
| 床の長方形 | 床にタイル状の暗い矩形 | `P_Floor_02` の中央パネル（`Object`）に**壁のマテリアル**が当たり、さらに床面より 10.4cm 低かった。173枚 |
...
**画面上の異物は、まず「ジオメトリか光か」を切り分ける。**
床の長方形を追ったとき、汚しデカール・ライトマップの継ぎ目・タイルの明るさむら、
と3つの見立てが立った。`VisualDiagnostics` が通常描画と ID 描画を2枚保存するので、
**ID 側に長方形が出るか**を見れば一発で切り分く（出ればジオメトリ）。
色→名前のパレットもログに出るので、画像の任意の座標から名前を逆引きできる。
正体は `P_Floor_02` の中央パネルに壁のマテリアルが当たっていたことだった。

**「もう直っているなら次へ」と書くと、2つ目の修正に永久に到達しない。**
M19 は材質を直した後、同じ判定で弾かれて高さの修正が走らなかった。
M15 の繋ぎ直しも同じ形で空振りしている。
**1つのループで2つ直すなら、判定も2つに分ける。**
...
| 床の上にある物を調べる | `FloorDiagnostics` | 約1分 |
| 床の中央パネルを直す（要ベイク） | `M19FloorPanelFix` → `M9BakedLightingPass.BakeAll` | 約7分 |
