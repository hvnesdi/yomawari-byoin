...
- 日時: 2026-07-30
- フェーズ: **M1〜M11 完了。ゲームは通しで動作し、見た目は一括で再現できる**
- 最新コミット: `2903697`（`origin/main` と一致・未コミットの変更なし）
...
| スキャン小物 | Poly Haven の CC0 実写スキャン10種を57個配置（椅子・スチール棚・段ボール・木箱・樽・清掃看板・ワゴン・時計） |
...
2. **スポーン位置**。自動で「一番開けている方向」を向かせたが、プレイヤー自体が
   壁際の角に置かれていて起動直後の画面が壁で埋まる。CLAUDE.md の想定は
   「薄暗い病室で目が覚める」なので、病室の中に置き直すのが本筋
3. **明るさのバランス**。実測は 平均輝度 0.253 / 最大 0.353 / 暗部(<0.15) 13%。
   「一様に薄暗い」寄りで、光の溜まりと闇のコントラストは弱い。
   調整値は `M3AtmospherePass.MoodFor()` に集約してある:
   - `ambient` … 暗さの下限　`fogDensity` … 奥行きの潰れ　`lightScale` … コントラスト
...
「壁に白い矩形」を3回追い、原因は毎回別だった（アルベド1.0超 → デカールの
ブレンド係数 → 白背景テクスチャ）。道具自体も2回疑うべきだった。
