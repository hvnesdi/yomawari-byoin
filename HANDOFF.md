...
### 🔴 未解決で引き継ぐもの：タイル壁の白い矩形

暗い廊下のタイル壁に、明るい矩形が複数枚浮いて見える（`Screenshots/PlayMode_1F.png`）。
**原因が特定できていない。** 以下は調べて「違った」ことの記録なので、同じ道を辿らないこと。

| 疑ったもの | 結果 |
|-----------|------|
| テクスチャ未設定で白くなっている | ✗ 該当したのは蛍光灯カバー・NPC・時計盤のみ |
| デカールのアルファが不透明 | ✗ 透明率 67〜100%、正常 |
| デカールの RGB が明るすぎる | ✗ 血・カビ・傷は輝度22〜31/255 と十分暗い |
| 漆喰(Mat_Walllime01_C)とタイルの Zファイティング | ✗ 漆喰を2cm奥へ逃がしても変化なし（この変更は revert 済み） |
| マテリアルの自己発光 | △ `Mat_Walllime01_C` の発光は実在し解除した（右の壁は暗くなった）が、矩形は残った |

**オブジェクトID描画による自動特定を実装したが、これも失敗している。**
`VisualDiagnostics.IdentifyFromCamera` はエディット/プレイどちらでも
「輝度0.72超のピクセル 0件」を返すのに、同じコードで保存したスクリーンショットには
はっきり白い矩形が写っている。**診断ツール側が壊れている**ので、まずそこを疑うこと。

**次にやるべきこと（人がやれば数秒）**:
Unity エディタで `Hospital.unity` を開き、Scene ビューで白い矩形を直接クリックする。
Inspector にオブジェクト名とマテリアルが出るので、それで確定する。
...
### M3: ホラーゲームとして見られる
5. ✅ 照明・フォグ・自己発光 → `M3AtmospherePass` で対応済み（フロアごとに暗さを設定）
6. 🔴 **白い矩形の特定**（上記「未解決で引き継ぐもの」参照。エディタでクリックするのが最短）
7. 病室天井の誤マテリアル（青いテクスチャ）— 未着手
8. 暗くした結果、**逆に暗すぎないか**の確認。1F は tutorial なので操作を覚えられる明るさが要る。
   数値は `M3AtmospherePass.MoodFor()` に集約してあるので、そこだけ触れば調整できる
9. 放置されている `Assets/Editor/HospitalPackFixV2.cs` の扱い

### M4: 整理
10. `git push`（main がローカルに8コミット先行）、`claude/*` 約30本の削除
11. マルチプレイ（Steam P2P）の実動作確認。`NetworkManager` は一度も実行されていない
