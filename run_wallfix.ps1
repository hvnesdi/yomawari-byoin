# Normalise wall panel heights, then capture a playtest screenshot. ASCII-only.
...
$log = Join-Path $logDir 'wallfix.log'
if (Test-Path $log) { Remove-Item $log -Force }

$proc = Start-Process -FilePath $unity -PassThru -Wait -ArgumentList @(
    '-batchmode', '-projectPath', $project,
    '-executeMethod', 'M3WallHeightFix.RunBatch',
    '-logFile', $log, '-quit'
)
Write-Host "Unity exit: $($proc.ExitCode)"
if (Test-Path $log) {
    Select-String -Path $log -Pattern 'error CS|\[M3WallHeightFix\]|Exception' |
