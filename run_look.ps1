# Apply the rendering look pass, then capture. ASCII-only.
...
function Run-Pass {
    ...
    if (Test-Path $log) {
        Select-String -Path $log -Pattern 'error CS|Compilation failed|\[M5\]|^\s{2}[a-zA-Z?]|Screenshot saved|Exception' |
            ForEach-Object { Write-Host $_.Line }
    }
}

Run-Pass -method 'M5LookPass.RunBatch' -logName 'm5_look.log'
if ($script:LastExit -ne 0) { exit $script:LastExit }

Run-Pass -method 'HospitalPropsAndCharacters.CaptureFour' -logName 'm5_capture.log'
